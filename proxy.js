...
// ---- EXPERT BRIEFING: weekly roundtable (generated by Cowork scheduled task) -
/**
 * The briefing is written to weekly-brief.json by the weekly Cowork scheduled
 * task (Monday 8am). This route simply serves that file. If none exists yet,
 * returns a placeholder. The briefing is not generated by the proxy — it comes
 * from a scheduled Claude session that fetches live data, synthesises expert
 * commentary, and writes the JSON file, then git-pushes so Render redeploys.
 */
async function fetchBriefing() {
  const briefPath = join(__dir, "weekly-brief.json");
  ...
  return {
    generatedAt: null,
    nextBriefing: null,
    note: "No briefing yet — the weekly expert report generates every Monday at 8am.",
    experts: [],
    sections: [],
    thesisEffects: [],
    actionItems: [],
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (url.pathname === "/api/briefing") {
      return send(res, 200, await fetchBriefing());
    }
